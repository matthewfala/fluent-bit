...
struct flb_event_loop {
    int n_priorities;
    struct mk_event_loop *primary_evl;     /* contains secondary evls */
    struct mk_event_loop **secondary_evls; /* contains priority specific events */
    struct mk_event *secondary_evl_events; /* events for evl */
};

/* create event loop */
static inline struct flb_event_loop *flb_event_loop_create(int priorities, int size) {
    struct flb_event_loop *evl;
    int i;

    /* create event loop */
    evl = flb_calloc(sizeof(struct flb_event_loop *)); /* mk_event_loop_create(priorities); */
    if (!evl) {
        flb_errno();
        return NULL;
    }

    /* load primary priority loop and register secondary loops */
    evl->primary_evl = mk_event_loop_create(priorities);
    if (!evl->primary_evl) {
        flb_event_loop_destroy(evl);
        return NULL;
    }

    /* initiate secondary priority loops */
    evl->secondary_evls = priorities;
    evl->secondary_evls = flb_calloc(priorities * sizeof(void *));
    if (!evl->secondary_evls) {
        flb_errno();
        flb_event_loop_destroy(evl);
        return NULL;
    }
    evl->secondary_evl_events = flb_malloc(priorities * sizeof(struct mk_event));

    /* load secondary priority loops */
    for(i = 0; i < priorities; ++i) {
        evl->secondary_evls[i] = mk_event_loop_create(size);
        if (!evl->secondary_evls[i]) {
            flb_event_loop_destroy(evl);
            return NULL;
        }

        /* register in primary evl */
        MK_EVENT_NEW(&evl->secondary_evl_events[i]);
        mk_event_add(evl->primary_evl, evl->secondary_evls[i]-> /* Blocker. no access to file descriptor on lib event... Windows. */
    }
...
static inline void flb_event_loop_destroy(struct flb_event_loop *evl)
{
    int i;
    if (evl->secondary_evls) {
        for (i = 0; (i < evl->n_priorities) && (evl->secondary_evls[i] != NULL); ++i) {
            mk_event_loop_destroy(evl->secondary_evls[i]);
        }
        flb_free(evl->secondary_evls);
    }
    if (evl->primary_evl) {
        mk_event_loop_destroy(evl->primary_evl);
    }
    if (evl->secondary_evl_events) {
        flb_free(evl->secondary_evl_events);
    }
    flb_free(evl);
}
