...
void flb_test_aws_ec2_tags_present() {
    ...
    name = flb_sds_printf(&name, "flb_test_aws_ec2_tags_present\0");
    ...
    output = get_output();
    if (output) {
        char *result = strstr(output, "\"Name\":\"my_ec2_instance\"");
    ...
    set_output(NULL);
    // TODO: Fix destroying the aws client mock generator.
    //     It is commented out to avoid segvs.
...
void flb_test_aws_ec2_tags_404() {
    ...
    ops = flb_calloc(1, sizeof(struct flb_filter_aws_init_options));
    name = flb_sds_create_size(100);
    name = flb_sds_printf(&name, "flb_test_aws_ec2_tags_404\0");
    ops->client_generator = client_generator;
    ops->name = name;
